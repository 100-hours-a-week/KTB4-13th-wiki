...
sources:
  - 모델-API-설계.md
limit_exempt: 원본 8개 엔드포인트의 필드표·에러표·규칙 문장을 자르지 않고 전부 보존하기 위해 250줄 상한을 넘김 (변환 승인 2026-09-17)
---
...
3. LLM이 후보 중 3권을 고르고 카드마다 한 줄 이유(`reason_short`), 긴 이유(`reason_long`), 근거(`match_basis`)를 **한 번의 호출로** 쓴다. 고른 순서가 곧 rank이며, match_score는 후보 점수 그대로라 내림차순이 아닐 수 있다
...
**`spec` 스키마**. 서버가 이 형식을 검증한다. (예시: [부록 - 3.3](appendix.md#33-예시-대화형-도서-추천-post-recommendationschat))
...
**초기 spec**. 새 스레드의 첫 턴, 그리고 422 이후 초기화할 때 BE가 보내는 값이다. (예시: [부록 - 3.3](appendix.md#33-예시-대화형-도서-추천-post-recommendationschat))

> **`buttons`, `degraded`는 V1 일반 턴에도 항상 존재한다.** 일반 턴은 보통 `buttons: []`, `degraded: false`.

**출력**

표지를 한 권으로 특정한 경우, 도서 소개 문장·유사 도서 카드·후속 동작 목록을 함께 반환한다(이미지 턴, V2). **인식 후보가 여럿인 경우**엔 확정하지 않고 후보를 최대 3개 제시해 사용자가 고르게 한다. 인식 실패면 200에 `candidates: []`와 안내 `reply`로 응답한다.
...
**입력**. 쿼리 파라미터로 받는다. 요청 본문은 없다. (예시: [부록 - 3.4](appendix.md#34-예시-개인화-추천-목록-get-recommendationsfeed))
