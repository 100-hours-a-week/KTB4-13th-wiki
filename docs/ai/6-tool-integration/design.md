...
3. tool 세트가 **동적·대규모**가 되어 수기 디스크립터 관리가 불가능해진다
    ...
    2. 단계 4는 2026-09-21에 LangGraph 도입을 확정해 그 트리거 ⓑ는 더 쓰지 않는다. 이 MCP 재평가 트리거는 그와 별개로 그대로 유효하다.
...
| 노드 | 설명 |
|---|---|
| 검색 모듈 ① | 키워드(pg_trgm)+벡터→제목 완전 일치 우선+RRF→랭킹 |
| 추천·취향 모듈 ③④⑤⑥ | 해석·후보검색·재랭킹·이유·취향추출·프로필 |
| 쇼핑 에이전트 ⑦ (V2) | 지시어해소→의도분류→tool루프→grounding |
| 모델 게이트웨이 | complete() · embed() · vision_encode() · ocr() · 어댑터·폴백체인·서킷·응답캐시·레이트리밋(폴백·서킷·캐시·레이트리밋은 V1 미구현) |
...
- **성능(실측·단계 2)**: 이유 1건 p95 **1.3초**, TTFB p95 1.0초 (한→해외 RTT 포함). ③ chat 1턴 SLO 4초(후에 8초로 다시 잡음, AI-7) 안에 여유(카드 3장 한 호출이라 출력 토큰↑ 여유분 확인 필요). ⑤는 야간 배치라 지연 무관
...
- **자체 vs API**: 이미 자체가 이김.
    ...
    - 모델 회귀 시 되돌리기는 설정 변경이 아니라 모델 교체 절차를 따른다: 도서 임베딩 전량 재적재 + BE의 전 사용자 ⑥ 재호출 + 기억 벡터 재생성(ERD §5).
...
- **폴백 체인** (V1 미구현): `[llm_primary, llm_secondary]` 설정으로 순서 지정. 앞이 서킷 오픈이면 다음으로. 임베딩은 체인을 두지 않는다(다른 모델 벡터는 기존 인덱스와 섞을 수 없다)
- **서킷 브레이커** (V1 미구현): 프로바이더별 실패율·지연 임계 초과 시 오픈 → 폴백 or 강등. 임베딩 장애는 폴백 없이 바로 계약 헤더(`X-Degraded: keyword-only`/`rule-only`), LLM 장애는 ③ 본문 `degraded: true`
- **응답 캐시** (V1 미구현): 임베딩은 텍스트 해시 키(선택). **LLM 이유 응답은 캐시하지 않는다** 긴 이유 보관은 BE 몫(명세 §1·③). TTL·스토리지 §8
- **레이트리밋** (V1 미구현): 프로바이더별 토큰 버킷(RPM/RPD). 초과 시 429 매핑 + `Retry-After`
- **타임아웃·재시도**: 기본 타임아웃(생성 30s / 임베딩 5s), 재시도는 idempotent 호출만 + 지터 백오프
- **비용 관측** (V1 미구현): 호출당 토큰·프로바이더·`purpose` 로깅 → 일일 지출 대시보드, 손익분기 추적
...
| 추가 대상 | 절차 | 호출부 영향 |
|---|---|---|
| **새 모델 프로바이더** (LLM 교체, 소형 LLM 자체서빙) | 게이트웨이 어댑터 1개 + 폴백 체인 설정 | 없음 (LLM 한정. 임베딩 모델 교체는 어댑터 교체에 더해 도서 임베딩 전량 재적재·BE의 ⑥ 재호출·기억 벡터 재생성이 필요하다, ERD §5) |
...
- 프로바이더·tool 변경이 어댑터/디스크립터로 국소화. LLM 호출 지점이 온라인 1곳(③)·야간 1곳(⑤)으로 명시적(V2에서 ⑦·표지 추가)
    ...
    - 2026-09-21 이후 ③⑤는 LangChain, ⑦은 LangGraph로 구현하므로 위 "프레임워크 추상화 뒤에 숨지 않음"은 더는 해당하지 않는다.
