> 🚧 작성중 · 담당 미정 · 수정 2026-09-18 · 원본 [서비스-인프라-확장성과-모니터링-설계](https://github.com/100-hours-a-week/KTB4-13th-wiki/wiki/%EC%84%9C%EB%B9%84%EC%8A%A4-%EC%9D%B8%ED%94%84%EB%9D%BC-%ED%99%95%EC%9E%A5%EC%84%B1%EA%B3%BC-%EB%AA%A8%EB%8B%88%ED%84%B0%EB%A7%81-%EC%84%A4%EA%B3%84)
...
**임베딩은 자체 서빙한다.** 외부 임베딩 API가 검색 지연의 대부분을 차지하고 처리량이 쿼터에 묶이는데, 자체 서빙으로 바꾸면 지연이 **99배** 빨라진다(4-1). 한국어 검색 품질이 앞서는 오픈 모델을 API로는 살 수 없기도 하다. 그래서 임베딩 컨테이너는 후속 과제가 아니라 **V1 배포 구성에 포함된다.** 모델은 e5-small을 잠정으로 두고 bge-m3와 비교해 확정한다.
...
**V1은 고정 구성이다.** 자동 확장 없이, 부하가 몰리면 앱이 서비스 토큰 단위로 429와 `Retry-After`를 돌려준다. DB와 LLM 예산을 지키는 최후 방어선이다. V1 피크는 AI 경로를 모두 합쳐 0.1 rps 안팎이라(1-2) 이 구성으로 충분하다.
...
| 외부 LLM 한도 초과 | ③⑤⑦ 모두 503 + `Retry-After` | 챗봇이 잠시 응답하지 않고 BE가 재시도를 안내 | 503 비율 | 쿼터가 풀리면 자동 |
| `embedding` 중단 · 벡터 인덱스 이상 | ①은 키워드 검색만(`X-Degraded: keyword-only`). 인덱스 이상이면 ④도 규칙 점수만(`rule-only`) | 의미 검색·개인화 약화 | 축소 응답 비율 | Docker 재시작 정책으로 재기동. 인덱스 이상은 재색인 |
...
| 트래픽 급증 | 초과분은 429 + `Retry-After` | 일부 429 후 재시도 성공 | 429 비율 | 자동 확장 없음 |
| LLM 예산 소진 | 80%에서 알림, 100%에서 저가 모델로 전환. 저가 모델도 쓸 수 없으면 규칙 응답(`degraded: true`) | 이유 문장 품질 저하 | 예산 소진율 | 예산 재배분 판단 |
...
| 임베딩 | 자체 서빙 CPU 컨테이너. e5-small(384차원) 잠정, bge-m3와 비교해 확정(4-1) | 외부 API보다 지연 99배·처리량 20배 개선. 쿼터에서 벗어난다 | **외부 임베딩 API**: 지연 573ms, 쿼터 제한. **GPU**: CPU로 충분하다 |
...
| 축 | 판정 기준 |
|---|---|
| 응답 시간 | 검색 p95 200ms, 홈 피드 p95 300ms, 챗봇 턴 p95 8초. 생성 30초 초과는 504로 끊는다 |
| 데이터 신선도 | 아래 표의 허용 지연 |
...
| 배치 | 초기 적재 진행률 | `ingest job` | 적재 중에는 검색이 `keyword-only`다 |
...
| 즉시 | LLM 예산 소진율 100% | 저가 모델 전환이 켜졌는지 확인 |
| 10분 내 | `replication_lag_seconds` > 300초 | 가격·재고·이력의 5분 허용 지연 위반 |
| 10분 내 | 검색 p95 > 200ms, 챗봇 턴 p95 > 8초 (5분) | 응답 시간 목표 위반 |
| 10분 내 | 축소 응답 비율 > 5% (10분) | 임베딩 컨테이너나 LLM 중 하나가 죽어 있다 |
| 10분 내 | 504 발생 | 생성 30초 초과 |
...
- **bge-m3는 품질 기대가 높지만 처리량이 8분의 1이다.** 결정은 실제 카탈로그의 recall@10으로 하고, 그때까지 e5-small을 잠정값으로 쓴다.
...
| C. 챗봇 | 동시 5 → 20 → 50턴 | 턴 p95, 504·503·429 건수, 진행 중 요청 수 | 턴 p95 ≤ 8초, 504 0건 |
...
| **임베딩 모델 교체** | 임베딩 컨테이너의 이미지 태그를 바꾸고 전량 재색인한다(5만 권 약 5.5분) ² |
| **복제 중단** | 응답이 전부 정상이라 사용자 신고로는 늦다. `replication_lag_seconds` 알림이 5분 허용 지연 안에서 잡는다 |
| **예산 소진** | 임베딩이 자체 서빙이라 호출량에 비례하는 비용이 LLM 하나뿐이다. 80%·100% 알림과 저가 모델 전환으로 3개월 36만 원을 넘기지 않는다 |
...
² 자체 서빙이라 재색인에 호출 과금이 없다. 취향 벡터는 BE가 전 사용자에 대해 ⑥을 다시 불러 새로 계산한다. 이때 BE가 보관한 기억 벡터는 옛 모델 값이라 그대로 쓸 수 없다(차원이 다르면 400, 같으면 조용히 섞인다). 기억 벡터를 다시 만드는 경로는 ERD §7에서 정한다
...
---
> ✏️ 이 페이지는 레포 [docs/ai/7-infra-monitoring/design.md](https://github.com/100-hours-a-week/KTB4-13th-wiki/blob/main/docs/ai/7-infra-monitoring/design.md)에서 수정한다. 위키에서 직접 고치면 다음 반영 때 덮어써진다.
