> 🚧 작성중 · 담당 김세훈 · 수정 2026-09-18 · 짝 문서 [부록](AI-1-모델-API-명세-부록) · 원본 [모델-API-설계](https://github.com/100-hours-a-week/KTB4-13th-wiki/wiki/%EB%AA%A8%EB%8D%B8-API-%EC%84%A4%EA%B3%84)

**요약** BE ↔ AI 구간의 엔드포인트 8개와 공통 규약을 정의한다.
...
² 사용자 메시지를 읽어 추천 조건을 갱신하고 카드를 최대 3장 준다. 카드마다 한 줄 이유와 상세 페이지용 긴 이유를 한 번의 LLM 호출로 함께 만든다. (V2) 사진을 보내면 같은 요청에서 표지를 인식한다

³ 구매·도서관·리뷰는 아래 복제 테이블에서 직접 읽는다. 호출할 때마다 전체를 다시 계산한다
...
¹ 같은 키, 같은 본문이면 다시 계산하지 않고 저장 결과를 200으로 반환. 응답이 유실돼 재시도해도 프로필이 두 번 바뀌지 않게

² 매번 처음부터 다시 만들기 때문에 기억만 바뀌어도 온보딩 응답을 다시 보냄. 건너뛴 사용자는 {}

³ 매번 전체를 보내 처음부터 다시 계산. 작가 취향은 이 경로로만 들어옴(type: author). 최대 500건(넘으면 최근 500건만)
...
| context_cards | object[] | N | 직전 턴에 보여준 추천 카드 ¹ |
...
¹ “1번, 이거” 같은 지시 표현를 풀고 후보를 고르는 재료. 항목: book_id, rank, price, match_score, in_stock. 최대 10개, 초과는 400

² 재시도로 같은 요청이 두 번 와도 장바구니에 두 번 담기지 않게. 읽기만 하는 턴에서는 무시
...
| resolved_reference | object? | “이거, 1번”이 가리키는 책을 찾은 결과 { ref, book_ids, confidence } ³ |
| selection | object? | “예산 안에서 N권”처럼 조건 걸고 골라 담을 때만 { goal, constraints, chosen[], total_price, alternatives_considered }. 아니면 null |
| buttons | object[] | 호출자가 제시할 후속 동작 목록. 없으면 []. action 값은 5장 action 표 참고 |

¹ 없으면 []. 내부 후보 추천(recommendations.candidates)도 기록용으로 포함

² 내부 후보 추천은 { count, candidates[]: {book_id, match_score} }. 나머지 형식은 예시이며 BE와 합의해 확정

³ confidence(0–1, 내부 판정용) 0.6 미만이면 되물음(tool_calls: []). 해석할 게 없으면 null
...
flowchart TB
    ...
    BE -->|홈 진입| F3
    BE -->|쇼핑 요청| AG
    BE ==>|"단방향 복제 · MySQL → PostgreSQL"| V[(커머스 복제 테이블)]
    V -. 인기 .-> F1
    V -. 이력 · 인기 .-> F2
    V -. 이력 · 인기 .-> F3
    V -. 이력 .-> F5
    ...
    AG -. tool 호출 .-> COM[백엔드: 장바구니, 주문, 카탈로그]
```
...
- V1의 호출 방향은 백엔드에서 AI 서버로 한 방향이다. 역방향 호출은 V2 쇼핑 에이전트에만 있다. ⑤의 취향 기억과 ③의 긴 이유는 응답 본문으로 나가 BE가 저장하는데, 이는 복제가 아니라 반환값이며 **BE의 재고·권한 검증을 지나가게 하려는 것**이다.
...
---
> ✏️ 이 페이지는 레포 [docs/ai/1-model-api/spec.md](https://github.com/100-hours-a-week/KTB4-13th-wiki/blob/main/docs/ai/1-model-api/spec.md)에서 수정한다. 위키에서 직접 고치면 다음 반영 때 덮어써진다.
