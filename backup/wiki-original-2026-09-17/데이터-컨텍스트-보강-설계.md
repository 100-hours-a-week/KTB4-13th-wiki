...
```mermaid
flowchart TB
    EMB["② POST /embeddings<br/>bge-m3-2026q3 · dim 1024<br/>모든 축의 공통 좌표계 (도서·passage·centroid·preset)"]

    subgraph RET["retrieval 이 본체 — 이미 구현됨 (RAG의 'R'은 여기 있다)"]
        S["① POST /search<br/>BM25(pg_trgm/tsvector) + pgvector + RRF<br/>결과 0건 → ③ 전환 신호"]
        FEED["④ GET /recommendations/feed<br/>취향 centroid 유사도 + 규칙 점수<br/>※ passage 증강 없음 · 이유 문구 없음 (즉시성)"]
    end

    subgraph AUG["retrieval → augmented generation — 단계 5가 채우는 곳 (전부 ③ 안)"]
        C1["③ spec 델타 추출<br/>preset_vectors 얕은 RAG<br/>자유어 → 표준 태그 (422 감소)"]
        C2["③ 근거 생성 (카드 3장 확정 후에만)<br/>book_passage top-k 증강<br/>→ reason_short·reason_long·match_basis 한 호출<br/>→ 문장별 인용 검증 (grounding)"]
        C3["③ 표지 인식 (V2 · Visual RAG)<br/>사진 → OCR →(폴백) VLM → 검색어<br/>→ v_books 텍스트 검색 → 실제 book_id"]
    end

    subgraph GRD["grounding 대칭축 — 생성은 하지만 passage RAG는 아님"]
        AG["⑦ POST /agent/act<br/>발화 수치 = 그 턴 tool 결과에서만<br/>(③ passage 인용의 형제 · 단계 5가 ③에 대칭 적용)"]
    end

    subgraph FIT["검색 대상을 만들고 유지 — retrieval 인프라"]
        PROF["⑥ POST /preferences/profile<br/>취향 centroid + 태그 점수 생성<br/>(③④ 개인화 검색의 기준)"]
        EXT["⑤ POST /preferences/extractions<br/>취향 문장 추출 + 임베딩<br/>(다음 ⑥의 입력)"]
        HL["⑧ GET /health<br/>replication_lag_seconds · 벡터 인덱스 상태"]
    end

    BP[("book_passage<br/>AI Postgres · pgvector<br/>국중도 소개·목차 (+리뷰 뷰(데이터) 확보 시 리뷰 본문)")]
...
flowchart TD
    subgraph INGEST["색인 (오프라인 / 야간) — AI 소유 배치"]
        NL["국중도 SEOJI 원본<br/>BOOK_INTRODUCTION·BOOK_TB·BOOK_SUMMARY (HTML)"] --> CLEAN["정제<br/>태그 제거 · 첫 완전문장부터 컷"]
        CLEAN --> CHUNK["청크 분할<br/>문단 / 목차항목 단위"]
        REV["리뷰 본문 (v_book_reviews 뷰 확보 시 · V1엔 없음)"] -.-> CHUNK
        CHUNK --> EMB2["② /embeddings<br/>bge-m3-2026q3 (dim 1024)"]
        EMB2 --> BP[("book_passage<br/>(AI PostgreSQL + pgvector — book_embeddings 옆)<br/>book_id · passage_type · text · vector · source_updated_at")]
        TAX["태그·카테고리 택소노미<br/>(ONBOARD-003 / 004 값)"] --> EMB2
        EMB2 --> TAXV[("preset_vectors<br/>tag_label · vector")]
    end

    subgraph REPL["BE MySQL → AI Postgres 단방향 복제"]
        BEDB[("BE MySQL<br/>커머스 원본")] --> VW[("AI Postgres 안 복제본<br/>v_books · v_user_* · v_book_popularity")]
    end

    subgraph TURN["③ 대화 추천 한 턴 (온라인)"]
        Q["입력: 발화 + 이전 spec + recent_turns"] --> SD["LLM: spec 델타 추출"]
        TAXV -. "자유어→표준 태그 정규화" .-> SD
        SD --> SPEC["서버 병합 spec (6키)<br/>위반 시 422"]
        SPEC --> HC["하이브리드 후보 검색<br/>BM25(pg_trgm/tsvector) + pgvector + RRF  (①과 동일 로직, 전부 같은 Postgres)"]
        VW -. "인기·이력" .-> SC
        HC --> SC["결합 스코어링<br/>질의 유사도 + 취향 centroid 유사도(⑥) + 인기(v_book_popularity)"]
        SC --> TOP{"카드 3장?"}
        TOP -- "아니오" --> FU["cards:[] + followup 되묻기"]
        TOP -- "예" --> RET["증강 검색<br/>각 book_id + spec.semantic<br/>→ book_passage top-k (소개·목차, +리뷰뷰 있으면)"]
        BP --> RET
        RET --> GEN["LLM (같은 한 호출):<br/>reason_short + reason_long + match_basis<br/>문장별 cited_passage_ids 출력"]
        GEN --> GRD["grounding 검증<br/>인용 없는 문장 제거 / 핵심 근거 미인용이면 템플릿 폴백"]
        GRD --> RESP["응답: cards(reason_short·reason_long·match_basis) + reply(SSE) + followup:null"]
        RESP -.->|"BE가 카드와 함께 저장"| BE1[("BE (MySQL)<br/>reason_long 보관 → 상세에서 그대로 렌더")]
    end
...
flowchart TD
    subgraph VQ["질의 (③에 image_ref 도착 — BE presigned URL)"]
        ...
        CONF -- "아니오<br/>(스타일 폰트·각도·부분 가림)" --> VLM["VLM: 표지 판독<br/>제목/저자/시리즈 단서를 구조화 텍스트로만 추출<br/>('무슨 책인지' 답하게 하지 않음)"]
        CONF -- "예" --> MATCH
        VLM --> MATCH["도서 DB 검색<br/>BM25(title+author) + pg_trgm 오타 보정"]
        MATCH --> TH{"단일 고신뢰 매칭?"}
        TH -- "예" --> R1["recognition.recognized:true + book_id<br/>(실제 DB의 book_id만)"]
...
| 임베딩 도메인 파인튜닝 | 분기별 | 질의–클릭 도서 쌍으로 한국어 도서 도메인 적응. bench recall 게이트 통과가 배포 조건. **전제**: 차원이 같아 `dim` 검사로는 옛 벡터를 거르지 못하므로, 기억 벡터·취향 프로필에 model 식별자가 생기기 전(ERD §7)에는 하지 않는다. 할 때는 전량 재적재 + BE의 ⑥ 재호출 절차를 따른다 |
