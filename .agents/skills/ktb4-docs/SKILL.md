...
6. `docs/README.md` 목차의 해당 묶음(`## AI` 등) 아래에 **문서 하나당 한 줄**(`- [제목](경로)`)로 링크를 추가한다(`(변환 후 추가)` 자리표시자면 지우고 채운다). 한 줄에 여러 문서를 `·` 로 잇지 않는다 — 여러 PR이 같은 줄을 고치면 머지 충돌이 나고, 한쪽만 선택해 풀면 다른 PR의 링크가 사라진다.
...
6. 검사를 실행한다.
   ...
   ```
   CI는 `preserve_check` 를 돌리지 않는다 (결정 #17). 변환 PR에서는 이 검사를 로컬로 실행하고 결과를 PR 본문에 적는다.
7. `제목 트리`, `누락 의심`, `뜻 바뀜 의심`, `숫자 누락` 이 0이 될 때까지 고친다. **원본 문장을 지워서 통과시키지 않는다.** 3단계를 켜지 않았는데 `문장 변경` 경고가 나오면 되돌린다 (`문체 변경`은 정보로만 집계된다).
8. `page-map.md` 상태를 `변환됨(확인 대기)` 로 바꾸고, `docs/README.md` 목차의 해당 묶음(`## AI` 등) 아래에 **문서 하나당 한 줄**(`- [제목](경로)`)로 링크를 추가한다(`(변환 후 추가)` 자리표시자면 지우고 채운다). 한 줄에 여러 문서를 `·` 로 잇지 않는다 — 여러 PR이 같은 줄을 고치면 머지 충돌이 나고, 한쪽만 선택해 풀면 다른 PR의 링크가 사라진다. PR을 만든다.
