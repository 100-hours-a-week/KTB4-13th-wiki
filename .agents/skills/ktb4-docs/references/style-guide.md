...
> 다만 **7번(다이어그램)과 "긴 섹션은 `<details>`로 접기"는 변환 문서에도 적용한다** — 둘 다 문장을 고치는 게 아니라 "보이는 위치·방식"만 바꾸는 것이라 원문 보존 원칙과 부딪히지 않는다 (원문에 이미 `<details>`가 있었으면 그대로 유지). 원문이 그 외 규칙을 크게 어기면 고치지 말고 파트에 알린다.
